...
class PlayerListAdapter(val itemClick: (Player) -> Unit): RecyclerView.Adapter<PlayerListAdapter.MyViewHolder>() {

    var datalist = mutableListOf<Player>()//리사이클러뷰에서 사용할 데이터 미리 정의 -> 나중에 MainActivity등에서 datalist에 실제 데이터 추가

    inner class MyViewHolder(private val binding: PlayerListItemBinding, val itemClick: (Player) -> Unit): RecyclerView.ViewHolder(binding.root) {
